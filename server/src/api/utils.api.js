...
router.get("/fetchCryptoSvg", async (req, res, next) => {
    let coinList = [
        "BTC",
        "LTC",
        "ETH",
    ...
    for (const symbol of coinList) {
        setTimeout(async () => {
            ...
            try {
                let result = null;
                try {
                    result = await axios.get(`https://cryptoflash-icons-api.herokuapp.com/svg/icon/${symbolName}`);
                } catch (error) {
                    result = await axios.get(`https://logo.chainbit.xyz/${symbolName}`);
                }
                ...
                console.log(error.message, symbolName);
            }
        }, 1000);
    }
    res.json({ lastSymbol });
